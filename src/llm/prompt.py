def select_prompt(name):
    prompt_extract = """
        ...
        以下の形式でJSONを出力してください：
        {
            ...
            "keyword": "検索キーワード。住所、駅名、お店ジャンルキャッチ、キャッチのフリーワード検索(部分一致)が可能。半角スペース区切りの文字列を渡すことでAND検索になる",
        }
        ...
        5. "keyword"に対する値は、他のキーで絞り込むことのできないキーワード（場所に関するキーワードや、お店のジャンルに関するキーワード）を指定してください。
            もし場所に関する指定がなかった場合は、キーワードに"文京区"を含めてください。
